...
class Cdao  //classe outil
{
    ...
    private function getObjetPDO() //mettre le DSN en paramètre formel
    {
        ...
        $strConnection = 'mysql:host=localhost;dbname=gsb_prof'; // DSN
    ...
    /*public function getTabObjetFromSql($squery) // $stype sera le type d'objet que l'on désire
    {
       ...
       $result = $sth->fetchAll(PDO::FETCH_OBJ); // fetchall met toutes les données dans un tableau d'objets du type en paramètre
       unset($lepdo);
       return $result; //$result est un tableau d'objet du type en parametre comportant tous les objets du type (All de fetchAll)
